...
class StockPickingMassAction(TransientModel):
    ...
    @api.model
    def _default_mass_unreserve(self):
        return self.env.context.get("mass_unreserve", False)
    ...
    mass_unreserve = fields.Boolean(
        string="Mass Unreserve",
        default=lambda self: self._default_mass_unreserve(),
        help="check this box if you want to check the availability of"
        " the selected Pickings.",
    )
